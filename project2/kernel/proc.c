...
{
  int slot_used[NTHREAD];
  for(int i=0; i<NTHREAD; i++) slot_used[i] = 0; // initialize

  struct proc* p;
  struct proc* cur_p = myproc();
  for(p = proc; p < &proc[NPROC]; p++){
    // acquire(&p->lock);
    if(p->state != UNUSED && p->mm == cur_p->mm && p->tf_va != 0){ // p and myproc shares process
      int p_slot = (TRAPFRAME - p->tf_va) / PGSIZE; // p_slot : where p occupies slot in page table?
      
      if(p_slot >= 0 && p_slot < NTHREAD){
        slot_used[p_slot] = 1;
      }
    }
    // release(&p->lock);
  ...
  for(int i=0; i<NTHREAD; i++){
    if(!slot_used[i]){ // find empty slot
      return TRAPFRAME - i*PGSIZE;
    }
  }
...
kclone(uint64 fn, uint64 arg, uint64 stack, int n_pages, int flags)
{
  ...
  if (flags & CLONE_VM) {
    ...
    // 1. increase refcount.
    mm_get(p->mm);
    // 2. initialize thread-group-related vairables.
    np->group_leader = p->group_leader;
    np->tgid = p->tgid;
    np->mm = p->mm;
    // 3. assign trapframe of np.
    acquire(&p->mm->lock);
    if((np->tf_va = find_free_tf_va(np->mm->pagetable)) == 0){
      release(&p->mm->lock);
      goto fail;
    }
...
kjoin(uint64 stack_addr)
{
  ...
  for(;;){
    ...
    sleep(p->group_leader, &wait_lock); // DOC : Wait-sleep.
  }
}
...
kexit(int status)
{
  ...
  if(p->tgid == p->pid){
    ...
    for(;;){
      int havesiblings = 0; // flag : if 1, no active siblings
      for(pp = proc; pp < &proc[NPROC]; pp++){
        acquire(&pp->lock);
...
kwait(uint64 addr)
{
  ...
  for(;;){
    ...
    havekids = 0;
    for(pp = proc; pp < &proc[NPROC]; pp++){
      // to implement wait() system call in mutlithread environment,
